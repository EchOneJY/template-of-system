...
import { queryArticleList, deleteArticle, changeTop, changePrivate } from '../../api';
...
const ArticleList = ({ ...props }) => {
    ...
    const columns = [
          ...
          render: (top, record, index) => {
            return (
              <Switch
                ...
                onChange={() => onSwitchChange(top === '00' ? '01' : '00', record, 'top')}
              />
            )
          }
        },
        ...
        },
        {
          ...
          render: (text, record) => (
            <span>
              <span className="link-button" onClick={() => props.history.push('/article/detail/' + record.id)}>编辑</span>
            </span>
          )
        }
    ]
   ...
   const queryArticle =  async() => {
    ...
    return (
        <div className="article-list">
            ...
            <Table pagination ={false} columns={columns} dataSource={dataSource} rowSelection={rowSelection} locale={{emptyText: '暂无数据' }} size="middle" bordered/>
            <div className="footer-box">
                <Button  icon="delete" onClick={() => handleDelete(selectedRows)} disabled={deleteState} className="delete-btn">删除</Button>
