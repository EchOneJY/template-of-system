    ...
    return (
        <div className="article-add">
            ...
            <Form.Item {...formItemLayout} label="可见">
                {getFieldDecorator('visible', { valuePropName: 'checked',initialValue: true })(<Switch />)}
            </Form.Item>
            ...
            <Form.Item {...formItemLayout} label="标签">
                ...
                <Select className="input-select" mode="multiple" placeholder="请选择标签">
                    {
                        tagsList.map(item => (<Option key={item._id} value={item.name}>{item.name}</Option>))
                    }
                </Select>
            )}
            </Form.Item>
