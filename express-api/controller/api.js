...
module.exports = {
    ...
    getArticleList: (req, res) => {
        ...
        p.find({}).sort({
            ...
            // 截取要显示的文章数据
            let perPageDatas = docs.slice(Number(articleIndex), Number(articleIndex) + Number(pageSize));
    ...
    login: (req, res) => {
        ...
        function checkCaptcha(code) {
            ...
            let sessCode = req.session.captcha ? req.session.captcha.toLowerCase() : null // 拿到之前存在session中的验证码
            // 进行验证
            if (code === sessCode) {
                // 成功后验证码作废
